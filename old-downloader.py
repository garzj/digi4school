...
startPageNo = int(input('Start downloading from page (1): ') or 1)
pageFileExtension = input('Choose a file extension for the pages: png/pdf/ps/svg (png): ') or 'png'
...
# extract the book suburl from the given ids
bookSubUrl = ''
...
try:
    ...
    session.post('https://a.digi4school.at/lti', data=getFormData(r.content))
except IndexError:
    print("<<< Error: The book url you provided seems to be invalid.")
    exit()
...
while True:
    ...
    try:
        if pageFileExtension in cairosvgsession.svg2:
            cairosvgsession.svg2[pageFileExtension](url=pageSvgUrl, write_to=fileSavePath, session=session, fillBG=True)
