...
def fetch(url, resource_type, session=None, fillBG=False):
    ...
    if session is not None:
        ...
        content = r.content
        if fillBG:
            backgroundTag = b'<rect width="100%" height="100%" fill="white"/>'
            content = re.sub(b'(<svg[^>]*>)()', b'\\1' + backgroundTag.replace(b'\\', b'\\\\'), content)
        return content
    else:
...
def read_url(url, url_fetcher, resource_type, session=None, fillBG=False):
    ...
    return url_fetcher(url, resource_type, session=session, fillBG=fillBG)
