...
class Node(dict):
    ...
    def __init__(self, element, style, url_fetcher, parent=None,
                 parent_children=False, url=None, unsafe=False, session=None):
        ...
        if parent_children:
            self.children = [
                Node(child.element, style, self.url_fetcher, parent=self,
                     unsafe=self.unsafe, session=self.session)
    ...
    def fetch_url(self, url, resource_type, session=None, fillBG=False):
        return read_url(url, self.url_fetcher, resource_type, session=session, fillBG=fillBG)
...
class Tree(Node):
        ...
        if tree_cache and kwargs.get('url'):
            ...
            if url and (url, element_id) in tree_cache:
                ...
                new_tree = Node(
                    ...
                    cached_tree.url_fetcher, parent, unsafe=unsafe, session=cached_tree.session)
    ...
    def __init__(self, **kwargs):
        ...
        if self_is_parent:
            ...
            tree = ET.fromstring(
                bytestring, parser=parser)
        self.xml_tree = tree
        root = cssselect2.ElementWrapper.from_xml_root(tree)
        style = parent.style if parent else css.parse_stylesheets(self, url, session=session)
        if element_id:
