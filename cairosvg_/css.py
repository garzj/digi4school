...
def find_stylesheets_rules(tree, stylesheet_rules, url, session=None):
        ...
        if rule.type == 'at-rule':
            if rule.lower_at_keyword == 'import' and rule.content is None:
                ...
                stylesheet = tinycss2.parse_stylesheet(
                    tree.fetch_url(css_url, 'text/css', session=session).decode('utf-8'))
                for rule in find_stylesheets_rules(
                        tree, stylesheet, css_url.geturl(), session=session):
                    yield rule
            # TODO: support media types
...
def parse_stylesheets(tree, url, session=None):
    ...
    for stylesheet in find_stylesheets(tree):
        for rule in find_stylesheets_rules(tree, stylesheet, url, session=session):
